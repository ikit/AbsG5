...
def init_pg(user, password, host, port, db):
    ...
    try:
        ...
        raise AbsgException(code="E000001", exception=ex)
    return con
...
try:
    ...
    raise AbsgException(code="E000002", exception=ex)
...
def generic_save(obj):
    ...
    try:
        ...
        if s: s.rollback()
        raise AbsgException(code="E100002", arg=[str(obj)], exception=ex)
...
def generic_count(obj):
    ...
    try:
        ...
        Session().rollback()
        raise AbsgException(msg="Unable to count how many object in the table", exception=ex)
...
def execute(query, loop=True):
    """
        Synchrone execution of the query in the shared session. If error occured, raise AbsgException
    """
    result = None
    s = Session()
    ...
    try:
        ...
        print ("EXCEPTION SQL !!!!")
        s.rollback()
        r = AbsgException(code="E100001", exception=ex)
        log_snippet(query, r)
        raise r
