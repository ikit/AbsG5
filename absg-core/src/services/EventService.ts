...
class EventService {
    ...
    public async getForMonth(year: number, month: number) {
        ...
        const result = await this.repo.query(q);
    ...
    private formatDateFromDatabase(event: EventG) {
        ...
        event.start = new Date(event.startYear, event.startMonth, event.startDay);
        event.start.setHours(event.startTime, -currentDate.getTimezoneOffset(), 0, 0);

        event.end = new Date(event.endYear, event.endMonth, event.endDay);
