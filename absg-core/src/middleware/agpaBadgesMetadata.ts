...
export interface BadgeMetadata {
    ...
    timing: 'direct' | 'progressive'; // direct = obtenable en 1 an, progressive = nécessite 3 ans
    icon: string;
    description: string;
    condition: string;
    color: string;
    // Pour les badges combo uniquement
    requires?: string[]; // Liste des badges prérequis
}

export const BADGES_METADATA: BadgeMetadata[] = [
    // ===== BADGES VOTANT (11) =====
    {
        badge: 'Le Patriote',
        type: 'voter',
 ...
 */
export function getBadgeMetadata(badgeName: string): BadgeMetadata | undefined {
 ...
 */
export function getBadgesByType(type: 'voter' | 'photographer' | 'combo'): BadgeMetadata[] {
 ...
 */
export function getBadgesByTiming(timing: 'direct' | 'progressive'): BadgeMetadata[] {
